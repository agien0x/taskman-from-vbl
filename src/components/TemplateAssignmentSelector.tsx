...
import { TemplateAssignees } from '@/components/TemplateAssignees';
...
interface TemplateAssignmentSelectorProps {
  templateId: string;
  ownerId?: string | null;
  compact?: boolean;
}

export const TemplateAssignmentSelector = ({ 
  templateId, 
  ownerId,
  compact = false 
}: TemplateAssignmentSelectorProps) => {
  const [profiles, setProfiles] = useState<Profile[]>([]);
  const [isOpen, setIsOpen] = useState(false);
  const { assignments, addAssignment, removeAssignment, updateRole } = useTemplateAssignments(templateId, ownerId);
      ...
      <PopoverTrigger asChild>
        {assignments.length > 0 ? (
          <Button variant="ghost" size="sm" className={cn("h-auto px-1", compact && "h-7")}>
            <TemplateAssignees assignments={assignments} maxVisible={3} />
          </Button>
        ) : (
