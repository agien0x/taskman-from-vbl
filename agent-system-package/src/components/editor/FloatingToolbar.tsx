...
export const FloatingToolbar = ({ editor }: FloatingToolbarProps) => {
  ...
  const updatePosition = useCallback(() => {
    ...
    // Получаем границы контейнера редактора
    const editorElement = editor.view.dom;
    const editorRect = editorElement.getBoundingClientRect();
    
    // Вычисляем позицию относительно редактора (не viewport!)
    const relativeTop = start.top - editorRect.top;
    const relativeLeft = ((start.left + end.left) / 2) - editorRect.left;
    
    // Позиционируем тулбар на 60px выше выделенного текста
    setPosition({ 
      top: relativeTop - 60,
      left: relativeLeft
    });
  ...
  return (
    <div
      className="absolute z-50 flex items-center gap-0.5 p-1 bg-popover border border-border rounded-md shadow-lg"
      style={{
        top: `${position.top}px`,
