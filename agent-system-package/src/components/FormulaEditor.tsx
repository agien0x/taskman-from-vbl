...
import { ModuleVersionCard } from "./ModuleVersionCard";
...
import { ModuleBadgeWithPopover } from "./ModuleBadgeWithPopover";
import { ConditionsBuilder } from "./ConditionsBuilder";
import { LogicElementAdder } from "./LogicElementAdder";
import { SortableLogicElementWithEdit } from "./SortableLogicElementWithEdit";
...
export const FormulaEditor = ({ 
  ...
  // Helper to use either passed toast prop or fallback to sonner toast
  ...
  // Conditionally use versions only if supabaseClient and toast are provided
  const versionsEnabled = !!(moduleId && agentId && supabaseClient && toastProp);
  const moduleVersionsHook = versionsEnabled 
    ? useModuleVersions(moduleId || "", agentId || "", supabaseClient, toastProp)
    : { versions: [], templates: [], saveVersion: async () => {}, saveAsTemplate: async () => {}, restoreVersion: async () => null, deleteVersion: async () => {} };
  
  const { versions, templates, saveVersion, saveAsTemplate, restoreVersion, deleteVersion } = moduleVersionsHook;
  const [showTemplateDialog, setShowTemplateDialog] = useState(false);
  const [templateName, setTemplateName] = useState("");
  
  const handleSave = async () => {
    if (!onSaveModule) {
      showToast("Функция сохранения не настроена", 'error');
      return;
    }
    try {
      ...
      if (firstIfStructure && firstIfStructure.conditionElements) {
        ...
        console.info('[FormulaEditor] Module ID:', moduleId);
        console.info('[FormulaEditor] Agent ID:', agentId);
        console.info('[FormulaEditor] Calling onSaveModule...');
        
        // Передаем обновлённый модуль явно, чтобы избежать гонки
        await onSaveModule?.(updatedModule);
        
        console.info('[FormulaEditor] onSaveModule completed successfully');
      }
      
      showToast("Триггер сохранен", 'success');
    } catch (error) {
  ...
  const duplicateFormulaElement = (id: string) => {
    ...
    showToast("IF-блок скопирован", 'success');
  };
  ...
  const handleTestTrigger = async (element: typeof formulaElements[0]) => {
    ...
    if (inputIds.length === 0) {
      showToast("Не выбран input для триггера", 'error');
      return;
    }
    ...
    if (!moduleId) {
      showToast("Не выбран модуль для активации", 'error');
      return;
    }

    const module = availableModules.find(m => m.id === moduleId);
    if (!module) {
      showToast("Модуль не найден", 'error');
      return;
    }
    ...
    try {
      ...
      if (selectedTaskId && supabaseClient) {
      ...
      if (!supabaseClient) {
        throw new Error("supabaseClient не предоставлен");
      }
      ...
      showToast("Ошибка при тестировании триггера", 'error');
