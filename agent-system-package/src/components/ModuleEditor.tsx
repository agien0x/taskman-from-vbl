import { AgentModule, JsonExtractorConfig, InputElement, DestinationElement, INPUT_TYPES, AVAILABLE_MODELS } from "../types/agent";
...
export const ModuleEditor = ({
  ...
  const handleTestModule = async () => {
    if (!testInput.trim()) {
      toast({ title: "Ошибка", description: "Введите тестовые данные", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      switch (module.type) {
        case 'model':
          if (!module.config.model) {
            toast({ title: "Ошибка", description: "Выберите модель LLM", variant: "destructive" });
            return;
          }
          testData = {
        ...
        default:
          toast({ title: "Ошибка", description: "Тестирование этого типа модуля пока не поддерживается", variant: "destructive" });
          return;
      }
      ...
      toast({ title: "Успешно", description: "Тестирование завершено" });
    } catch (error) {
      console.error("Error testing module:", error);
      toast({ title: "Ошибка", description: "Ошибка при тестировании модуля", variant: "destructive" });
  ...
  const allAvailableInputs = [
    ...availableInputs,
    ...getDynamicInputsFromModules()
  ];
  ...
  const renderModuleContent = () => {
    ...
    // Fallback for non-migrated modules with full UI
    switch (module.type) {
      case 'trigger':
        return (
      ...
      case 'model':
        return (
          <div className="space-y-3">
            ...
            <div className="space-y-3 pt-2">
              <Label className="text-sm">Тестирование модели</Label>
              <div className="space-y-2">
                <textarea
                  value={testInput}
                  onChange={(e) => setTestInput(e.target.value)}
      ...
      case 'json_extractor':
        return (
          <div className="space-y-3">
            ...
            <div className="text-xs text-muted-foreground bg-muted/30 p-2 rounded border mt-2">
              <strong>Outputs:</strong> {module.config.variables?.map((v: any) => v.name).join(', ') || 'Нет переменных'}
            </div>
          </div>
        );
      ...
      default:
        return (
          <div className="text-sm text-muted-foreground">
            Редактор для модуля типа {module.type} находится в разработке.
          </div>
        );
    }
  };
